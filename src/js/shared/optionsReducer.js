 ...
 */
export const reducer = (state = { nextID: 0, properties: [], sets: {} }, action) => {
    // Return the default state generated by this reducer if action or the type
    // doesn't exist.
    if (!action || !action.type)
        return state;

    // If we want to amend sets, use the sub-reducer.
    const setReducerActions = Object.keys(setsReducer.actions).map(action => setsReducer.actions[action]);

    // Delegate set operations to this other reducer.
    if (setReducerActions.includes(action.type))
